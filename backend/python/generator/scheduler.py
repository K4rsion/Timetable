...
from utils import load_data, show_timetable, set_up, write_solution_to_file
...
def evolutionary_algorithm(matrix, data, free, filled, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    for run in range(run_times):
        ...
        while stagnation < max_stagnation:
            ...
            # 10*n
            for i in range(len(costs_list) // 128):
...
def simulated_hardening(matrix, data, free, filled, groups_empty_space, teachers_empty_space, subjects_order,
    ...
    write_solution_to_file(matrix, data, filled, file_path, groups_empty_space, teachers_empty_space, subjects_order)
...
def main(file_path, file):
    ...
    data = load_data(file_path + file, teachers_empty_space, groups_empty_space, subjects_order)
    ...
    simulated_hardening(matrix, data, free, filled, groups_empty_space, teachers_empty_space, subjects_order, file_path)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Schedule generation program')
    parser.add_argument('file_path', type=str, help='Path to the input JSON file')
    args = parser.parse_args()
    file_name = "\\input.json"
    main(args.file_path, file_name)
